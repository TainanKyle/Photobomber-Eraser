...
class Stitcher:
    ...
    def drawMatches(self, imgs, matches_pos):
        ...
        for (img_left_pos, img_right_pos) in matches_pos:
                ...
                cv.circle(vis, pos_l, 3, (0, 0, 255), 1)
                cv.circle(vis, pos_r, 3, (0, 255, 0), 1)
                cv.line(vis, pos_l, pos_r, (255, 0, 0), 1)
...
class Blender:
    def linearBlending(self, imgs, img_ref):
        ...
        for i in range(hr): 
            ...
            # check the region of extracted people
            for j in range(wr):
                if (img_right_mask[i, j] == 0):
                    alpha_mask[i, j] = 1
